...
namespace Entidades
{
    public class Deuda
    {
        ...
        public bool BuscarNCliente(string nCLiente, out List<Detalle> listadoDeuda)
        {
            bool retorno = false;
            listadoDeuda = new List<Detalle>();
            foreach (Detalle item in this.detalles)
            {
                if (item.SonIgualesNCliente(nCLiente))
                {
                    retorno = true;
                    listadoDeuda.Add(item);
                }
        ...
        public bool BuscarDNI(string dni, int estado)
        {
            ...
            foreach (Detalle item in this.detalles)
            {
                if (item.SonIgualesDNI(dni) && item.Estado != estado)
                {
                    retorno = true;
                    break;
                }
        ...
        public bool BuscarNCliente(string nCliente, int estado)
        {
            bool retorno = false;
            foreach (Detalle item in this.detalles)
            {
                if (item.SonIgualesNCliente(nCliente) && item.Estado != estado)
                {
                    retorno = true;
                    break;
                }
